...
public:
     ...
     */
    QImage cvMatToImage(const cv::Mat& image) const;
     ...
     */
    cv::Mat imageToCvMat(const QImage& image) const;
     ...
     */
    void split(const cv::Mat& input, cv::Mat& outputLeft, cv::Mat& outputRight) const;

    /*!
     * \brief Applies a blur filter to an image.
     * \param image         Image which has to be blurred.
     * \param kernelWidth   Blurring kernel width.
     * \param kernelHeight  Blurring kernel height.
     */
    void blur(cv::Mat& image, int kernelWidth, int kernelHeight) const;
     ...
     */
    void sobel(cv::Mat& image, int destDepth, int derX, int derY, int kernelSize,  double scale, double delta) const;
     ...
     */
    void canny(cv::Mat& image, double histThrA, double histThrB, int aperture, bool gradientMagnitude) const;
     ...
     */
    const cv::Mat& disparityMap(cv::Mat leftImage, cv::Mat rightImage, const DisparityMapParameters* param = nullptr, StereoMode mode = StereoMode::STEREO_MODE_BM);

    const cv::Mat& depthMap(const cv::Mat& leftImage, const cv::Mat& rightImage, const DisparityMapParameters* param = nullptr, StereoMode mode = StereoMode::STEREO_MODE_BM);

    cv::Mat flann(const cv::Mat& leftImage, const cv::Mat rightImage);
